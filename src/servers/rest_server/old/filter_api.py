...
class FilterFunctions(ProcessQuery):
    ...
    def get(self):
        ...
        basepath = os.path.dirname(__file__)
        filters_path = os.path.join(basepath, "../filters/")
        files = self.filter_bank.import_filters_from_files(filters_path)
        #files = self.filter_bank.get_env_keys()
        functions = []
        for f in files:
            if f[0] != "_":
                functions += [{'name': f, 'code': self.filter_bank.get_code(filters_path, f), 'doc': self.filter_bank.get_docstring(f)}]
